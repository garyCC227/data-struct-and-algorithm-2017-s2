...
void putDLList(FILE *out, DLList L)
{
	...
	DLListNode *curr;
	if (L->curr != NULL) printf("Current: %s\n", L->curr->value);
...
void DLListBefore(DLList L, char *it)
{
	...
	if(L->nitems == 0){
		...
		L->curr = new;
		if(new->prev != NULL){
			new->prev->next = L->curr;
		}
		DLListNode *countfirst = L->curr;
		while(countfirst->prev != NULL){
			countfirst = countfirst->prev;
		}
		L->first = countfirst;

		DLListNode *countlast = L->curr;
